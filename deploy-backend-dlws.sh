...
ssh -o StrictHostKeyChecking=no $SSH_KEY $SERVER_USER@$SERVER_IP "source ~/.bashrc 2>/dev/null || true; source /etc/profile 2>/dev/null || true; AAPANEL_NODE=\$(ls -d /www/server/nodejs/v*/bin 2>/dev/null | tail -n 1); export PATH=\$AAPANEL_NODE:\$PATH:/usr/local/bin:/usr/bin:\$(npm config get prefix 2>/dev/null)/bin:~/.nvm/versions/node/\$(ls ~/.nvm/versions/node 2>/dev/null | tail -n 1)/bin; cd $SERVER_PATH && unzip -o backend-dlws.zip && rm backend-dlws.zip && npm install --omit=dev && (fuser -k 5010/tcp 2>/dev/null || lsof -ti:5010 | xargs kill -9 2>/dev/null || true) && (PM2_CMD=\$(command -v pm2 || which pm2 2>/dev/null || echo \"\"); if [ -z \"\$PM2_CMD\" ]; then npm install -g pm2 2>/dev/null || true; PM2_CMD=\$(command -v pm2 || echo \"\"); fi; if [ -n \"\$PM2_CMD\" ]; then \$PM2_CMD restart backend-dlws 2>/dev/null || \$PM2_CMD start server.js --name \"backend-dlws\"; \$PM2_CMD save 2>/dev/null || true; else pkill -f 'node.*server.js' || true; nohup node server.js > app.log 2>&1 & fi)"
