...
ssh -o StrictHostKeyChecking=no $SSH_KEY $SERVER_USER@$SERVER_IP "source ~/.bashrc 2>/dev/null || true; source /etc/profile 2>/dev/null || true; export PATH=\$PATH:/usr/local/bin:/usr/bin:\$(npm config get prefix 2>/dev/null)/bin:~/.nvm/versions/node/\$(ls ~/.nvm/versions/node 2>/dev/null | tail -n 1)/bin:/www/server/nvm/versions/node/\$(ls /www/server/nvm/versions/node 2>/dev/null | tail -n 1)/bin; cd $SERVER_PATH && unzip -o backend-dlps.zip && rm backend-dlps.zip && npm install --omit=dev && (PM2_CMD=\$(command -v pm2 || which pm2 2>/dev/null || echo \"\"); if [ -n \"\$PM2_CMD\" ]; then \$PM2_CMD reload all 2>/dev/null || \$PM2_CMD restart all 2>/dev/null || \$PM2_CMD start server.js --name \"backend-dlps\"; \$PM2_CMD save 2>/dev/null || true; else pkill -f 'node.*server.js' || true; nohup node server.js > app.log 2>&1 & fi)"
