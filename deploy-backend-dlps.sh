...
ssh -o StrictHostKeyChecking=no $SSH_KEY $SERVER_USER@$SERVER_IP "export PATH=\$PATH:/usr/local/bin:/usr/bin:~/.nvm/versions/node/\$(ls ~/.nvm/versions/node 2>/dev/null | tail -n 1)/bin:/www/server/nvm/versions/node/\$(ls /www/server/nvm/versions/node 2>/dev/null | tail -n 1)/bin; cd $SERVER_PATH && unzip -o backend-dlps.zip && rm backend-dlps.zip && npm install --omit=dev && (pm2 reload all || pm2 restart all || pkill -f 'node.*server.js' || true)"
