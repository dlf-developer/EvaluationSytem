...
function TeacherCoScholastic() {
  ...
  const handleSubmit = async (data) => {
    ...
    const response = await dispatch(TeacherCoScholasticComplete(payload));
    if (response?.payload?.message) {
      message.success(response?.payload?.message);
      const receiverId = formDataList?.createdBy?._id;
      const observerMessage = `${formDataList?.grenralDetails?.NameoftheVisitingTeacher?.name} has completed Co-Scholastic Form for ${formDataList?.grenralDetails?.className} | ${formDataList?.grenralDetails?.Section} | ${formDataList?.grenralDetails?.Subject}.`;
      const teacherMessage = `You have completed Co-Scholastic form for ${formDataList?.grenralDetails?.className} | ${formDataList?.grenralDetails?.Section} | ${formDataList?.grenralDetails?.Subject}.`;
      const activity = {
        observerMessage,
        teacherMessage,
        route: `/co-scholastic/report/${formDataList?._id}`,
        date: new Date(),
...
export default TeacherCoScholastic;
