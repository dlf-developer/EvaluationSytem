...
const BadgeText = ({ val }) => {
  ...
  return (
    <Text style={[s.badge, { backgroundColor: bg, color }]}>{val || "—"}</Text>
  );
};
...
const form1Score = (form) => {
  ...
  const vals = Object.values(form).filter((v) =>
    ["Yes", "No", "Sometimes", "N/A"].includes(v),
  );
  const score = Object.values(form).reduce(
    (s, v) => s + (v === "Yes" ? 1 : v === "Sometimes" ? 0.5 : 0),
    0,
  );
...
const WingCoordinatorDoc = ({ data }) => {
  ...
  const dateRange =
    range?.length === 2 ? `${fmt(range[0])} – ${fmt(range[1])}` : "—";
    ...
    <Document>
      ...
      <Page size="A4" style={s.page}>
        <View style={s.container}>
          <PageHeader
            title="Monthly Report - Wing Coordinator Report"
            sub={`Date Range: ${dateRange}  |  Classes: ${classes}`}
            n={1}
            total={
              1 +
              form1.length +
          ...
          <View style={s.table}>
            <View style={s.tableRow}>
              <View style={[s.th, s.col50]}>
                <Text>Section</Text>
              </View>
              <View style={[s.th, s.col50Last]}>
                <Text>Records Selected</Text>
              </View>
            </View>
            {[
              ...
              >
                <View style={[s.td, s.col50]}>
                  <Text>{label}</Text>
                </View>
                <View style={[s.td, s.col50Last]}>
                  <Text>{count}</Text>
                </View>
              </View>
            ))}
      ...
      {form1.map((item, idx) => {
        ...
        const keys = item.teacherForm
          ? Object.keys(item.teacherForm).filter(
              (k) => !["totalScore", "OutOf", "ObservationDates"].includes(k),
            )
          : [];
        return (
          <Page key={`f1-${idx}`} size="A4" style={s.page}>
            <View style={s.container}>
              <PageHeader
                title={`Fortnightly Monitor — Record ${idx + 1}`}
                n={2 + idx}
                total="—"
              />
              {/* meta */}
              <View style={s.metaGrid}>
                <View style={s.metaItem}>
                  ...
                  <Text style={s.metaValue}>{item.teacherID?.name ?? "—"}</Text>
                </View>
                <View style={s.metaItem}>
                  <Text style={s.metaLabel}>Observer</Text>
                  <Text style={s.metaValue}>{item.userId?.name ?? "—"}</Text>
                </View>
                <View style={s.metaItem}>
                  <Text style={s.metaLabel}>Class / Section</Text>
                ...
                <View style={s.metaItem}>
                  ...
                  <Text style={s.metaValue}>{fmt(item.date)}</Text>
                </View>
                <View style={s.metaItem}>
                  <Text style={s.metaLabel}>Teacher Submitted</Text>
                  <Text style={s.metaValue}>
                    {fmt(item.TeacherSubmissionDate)}
                  </Text>
                </View>
                <View style={s.metaItem}>
                  <Text style={s.metaLabel}>Observer Submitted</Text>
                  <Text style={s.metaValue}>
                    {fmt(item.ObserverSubmissionDate)}
                  </Text>
                </View>
              </View>
              {/* table */}
              <View style={s.table}>
                <View style={s.tableRow}>
                  <View style={[s.th, s.col60]}>
                    <Text>Question</Text>
                  </View>
                  <View style={[s.th, s.col20]}>
                    <Text style={{ textAlign: "center" }}>Teacher</Text>
                  </View>
                  <View style={[s.th, s.col20Last]}>
                ...
                {keys.map((key, ki) => (
                  <View
                    key={key}
                    style={ki === keys.length - 1 ? s.tableRowLast : s.tableRow}
                  >
              ...
              <View style={s.scoreRow}>
                <View style={s.scoreBox}>
                  <Text style={s.scoreLabel}>Teacher Score</Text>
                  <Text style={s.scoreValue}>
                    {tScore.score} / {tScore.total}
                  </Text>
                </View>
                <View style={s.scoreBox}>
                  <Text style={s.scoreLabel}>Observer Score</Text>
                  <Text style={s.scoreValue}>
                    {oScore.score} / {oScore.total}
                  </Text>
                </View>
              </View>
            </View>
      ...
      {form2.map((item, idx) => (
        <Page key={`f2-${idx}`} size="A4" style={s.page}>
          <View style={s.container}>
            <PageHeader
              title={`Classroom Walkthrough — Record ${idx + 1}`}
              n={2 + form1.length + idx}
              total="—"
            />
            <View style={s.metaGrid}>
              <View style={s.metaItem}>
              ...
              <View style={s.metaItem}>
                <Text style={s.metaLabel}>Observer</Text>
                <Text style={s.metaValue}>{item.createdBy?.name ?? "—"}</Text>
              </View>
              <View style={s.metaItem}>
                <Text style={s.metaLabel}>Class / Section</Text>
                <Text style={s.metaValue}>
                  {item.grenralDetails?.className} /{" "}
                  {item.grenralDetails?.Section}
                </Text>
              </View>
              <View style={s.metaItem}>
                <Text style={s.metaLabel}>Subject</Text>
                <Text style={s.metaValue}>
                  {item.grenralDetails?.Subject ?? "—"}
                </Text>
              </View>
              <View style={s.metaItem}>
                <Text style={s.metaLabel}>Topic</Text>
                <Text style={s.metaValue}>
                  {item.grenralDetails?.Topic ?? "—"}
                </Text>
              </View>
              <View style={s.metaItem}>
                <Text style={s.metaLabel}>Date of Observation</Text>
                <Text style={s.metaValue}>
                  {fmt(item.grenralDetails?.DateOfObservation)}
                </Text>
              </View>
            </View>
            ...
            {f2Sections.map((sec) => {
              ...
              const label =
                sec === "essentialAggrements"
                  ? "Essential Agreements"
                  : sec === "planingAndPreparation"
                    ? "Planning & Preparation"
                    : sec === "classRoomEnvironment"
                  ...
                  <View style={s.table}>
                    ...
                    {items.map((q, qi) => (
                      <View
                        key={qi}
                        style={
                          qi === items.length - 1 ? s.tableRowLast : s.tableRow
                        }
                      >
                        <View style={[s.td, s.col60]}>
                        ...
                        <View
                          style={[s.td, { width: "40%", alignItems: "center" }]}
                        >
            ...
            <View style={s.scoreRow}>
              ...
              <View style={s.scoreBox}>
                ...
                <Text style={s.scoreValue}>{item.Grade ?? "—"}</Text>
              </View>
              <View style={s.scoreBox}>
                <Text style={s.scoreLabel}>%</Text>
                <Text style={s.scoreValue}>{item.percentageScore}%</Text>
              </View>
            </View>
        ...
        <Page size="A4" style={s.page}>
          <View style={s.container}>
            ...
            <Text style={s.sectionHead}>
              Notebook Checking Proforma ({form3.length} records)
            </Text>
            <View style={s.table}>
              <View style={s.tableRow}>
                <View style={[s.th, s.col40]}>
                  <Text>Teacher</Text>
                </View>
                <View style={[s.th, s.col30]}>
                  <Text>Class</Text>
                </View>
                <View style={[s.th, s.col20Last, { width: "30%" }]}>
                  <Text>Date</Text>
                </View>
              </View>
              {form3.map((f, i) => (
                <View
                  key={i}
                  style={i === form3.length - 1 ? s.tableRowLast : s.tableRow}
                >
                  <View style={[s.td, s.col40]}>
                    <Text>{f.teacherID?.name ?? "—"}</Text>
                  </View>
                  <View style={[s.td, s.col30]}>
                    <Text>{f.grenralDetails?.className ?? "—"}</Text>
                  </View>
                  <View style={[s.td, { width: "30%" }]}>
                    <Text>{fmt(f.createdAt)}</Text>
                  </View>
                </View>
              ))}
            </View>
      ...
      {form4.length > 0 && (
        <Page size="A4" style={s.page}>
          <View style={s.container}>
            ...
            <Text style={s.sectionHead}>
              Learning Progress Checklist ({form4.length} records)
            </Text>
            <View style={s.table}>
              <View style={s.tableRow}>
                <View style={[s.th, s.col50]}>
              ...
              {form4.map((f, i) => (
                <View
                  key={i}
                  style={i === form4.length - 1 ? s.tableRowLast : s.tableRow}
                >
                  <View style={[s.td, s.col50]}>
                    <Text>{f.teacherId?.name ?? f.userId?.name ?? "—"}</Text>
                  </View>
                  <View style={[s.td, s.col50Last]}>
                    <Text>{fmt(f.createdAt)}</Text>
                  </View>
